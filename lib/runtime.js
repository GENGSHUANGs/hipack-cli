...
const Runtime = module.exports.Runtime = class Runtime {
    ...
    exists() {
        return fs.existsSync(path.join(this.dist,'package.json'));
    }

    remove(){
        del.sync([path.join(this.dist,'*')],{dot: true ,force:true});
    }
    ...
    install(callback) {
        var proc = spawn(global._hipack_npm_command, ['install', '--registry=https://registry.npm.taobao.org'], {
            cwd: this.dist,
            stdio: 'inherit',
        });
